        ...
        $(document).ready(function() {
            ...
            $('#adminTambahSppBtn').on('click', function() {
                data = {
                    ...
                    nominal: '',
                    btn: 'Tambah',
                    action: '/sppuk/public/admin-spp/store',
                }
            ...
            {
                $.ajax({
                    url: '/sppuk/public/admin-spp/getSppData',
                    data: {
                        id: id,
                    },
                    dataType: 'json',
                    ...
                    success: function(res) {
                        data = {
                            ...
                            nominal: res['nominal'],
                            btn: 'Ubah',
                            action: '/sppuk/public/admin-spp/update',
                        }
                        setSppModal(data)
                    },
                })
            ...
            $('#adminTambahKelasBtn').on('click', function() {
                data = {
                    ...
                    komp: '',
                    btn: 'Tambah',
                    action: '/sppuk/public/admin-kelas/store',
                }
            ...
            {
                $.ajax({
                    url: '/sppuk/public/admin-kelas/getKelasData',
                    data: {
                        id: id,
                    },
                    dataType: 'json',
                    ...
                    success: function(res) {
                        data = {
                            ...
                            btn: 'Edit',
                            action: '/sppuk/public/admin-kelas/update',
                        }
                        setKelasModal(data)
                    },
                })
            ...
            // petugas kelas manajemen
            $('.kelas-row').on('click', '#petugasEditKelasBtn', function () {
                const id = $(this).parents('.kelas-row').data('idkelas')

                petugas_getKelasData(id)
            })

            $('#petugasTambahKelasBtn').on('click', function() {
                data = {
                    title: 'Tambah Data Kelas',
                    id_kelas: '',
                    nama: '',
                    komp: '',
                    btn: 'Tambah',
                    action: '/sppuk/public/petugas-kelas/store',
                }

                petugas_setKelasModal(data)
            })

            function petugas_getKelasData(id)
            {
                $.ajax({
                    url: '/sppuk/public/petugas-kelas/getKelasData',
                    data: {
                        id: id,
                    },
                    dataType: 'json',
                    ...
                    success: function(res) {
                        data = {
                            ...
                            komp: res['kompetensi_keahlian'],
                            btn: 'Edit',
                            action: '/sppuk/public/petugas-kelas/update',
                        }
                        petugas_setKelasModal(data)
                    },
                })
            ...
            $('.kelas-row').on('click', '#petugasHapusKelasBtn', function() {
                const id = $(this).parents('.kelas-row').data('idkelas')

                $modal = $('#petugasDeleteKelasModal')
                $modal.find('input[type=hidden]').val(id)
            })

            function petugas_setKelasModal(data)
            {
                modal = $('#petugasKelasModal')

                modal.find('#petugasKelasModalLabel').html(data['title'])
                modal.find('form').attr('action', data['action'])
                modal.find('input[type=hidden]').val(data['id_kelas'])
                modal.find('#petugasKelasNamaInput').val(data['nama'])
                modal.find('#petugasKelasKompInput').val(data['komp'])

                modal.find('button[type=submit]').html(data['btn'])
            }
            // end of petugas kelas manajemen
        })
    </script>
</body>
